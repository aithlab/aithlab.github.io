...
excerpt: "Google Colab에서 PyTorch를 TPU의 single-core에서 실행하기"
toc: true
toc_sticky: true
toc_label: "목차"
use_math: true
...
각 버전이 무슨 차이인지는 정확히 모르겠지만, 나중에 TPU를 multi-core로 돌리기 위해선 `xrt==1.15.0`을 설치하도록 하자. ~~`20200220` 버전의 경우 single-core에서는 잘 작동하는데 multi-core에서는 오류가 발생하는 것으로 확인하였다.~~ 다시 확인해보니 오류가 `20200220` 버전도 multi-core에서 오류가 발생하지 않는다. 따라서 `xrt==1.15.0`과 `20200220` 버전 중 아무거나 설치해도 되는 듯 하다. (`nightly` 버전은 거의 사용을 안 하는 듯 하다.) 
