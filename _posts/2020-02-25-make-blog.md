...
<img src="{{ site.url }}{{ site.baseurl }}/assets/images/1.make_blog/1_3_blog_test_page.png">

나는 이 단계에서 다음의 오류가 발생하였다. 
...
단순히 rake라는 파일이 설치되지 않아 발생되는 오류 같은데, `bundle install`로 해결 할 수 있었다.  
...
앞서 clone해온 테마의 폴더로 이동하자. 
```shell
$ cd jekyll-theme-minimal-resume
```  

그리고 `jekyll serve` 명령어를 통해 로컬에서 페이지를 띄워보자. 
<img src="{{ site.url }}{{ site.baseurl }}/assets/images/1.make_blog/2_6_preview_blog.png">
